...
#define PLUGIN "[ZB3EX] ZClass: Sting Finger"
#define VERSION "2.0"
#define AUTHOR "Dias"
...
new const StabSound[] = "zombie_thehero/resident_stab.wav"
...
new g_Msg_Fov, g_synchud1, g_current_time[33], g_current_time2[33]
new g_can_tentacle[33], g_can_hj[33], g_hj_ing[33], g_temp_attack[33], m_iBlood[2]
new g_remaining_time[33] , g_remaining_time2[33]
...
public client_PreThink(id)
{
	...
	static target, body;
 	...
 	new Float: ShockDistance = zb3_get_user_level(id) > 1 ? float(TENTACLE_DISTANCE_ORIGIN) : float(TENTACLE_DISTANCE_HOST)
...
public cmd_drop(id)
{
	...
	if(!g_can_tentacle[id])
	{
		client_print(id, print_center, "%L", LANG_PLAYER, "ZOMBIE_SKILL_NOT_READY", zclass_desc , get_cooldowntime(id) - g_remaining_time[id])
...
public fw_CmdStart(id, uc_handle, seed)
{
	...
	if((CurButton & IN_RELOAD) && !(pev(id, pev_oldbuttons) & IN_RELOAD))
	{
		...
		{
			client_print(id, print_center, "%L", LANG_PLAYER, "ZOMBIE_SKILL_NOT_READY", zclass_desc , get_cooldowntime2(id) -  g_remaining_time[id])
			return PLUGIN_HANDLED
		}
...
public zb3_skill_show(id)
{
	...
	// HeavyJump Skill
	timewait2 = zb3_get_user_level(id) > 1 ? HEAVYJUMP_COOLDOWN_ORIGIN : HEAVYJUMP_COOLDOWN_HOST
	percent2 = floatround((float(g_current_time2[id]) / float(timewait2)) * 100.0)
	
	g_remaining_time[id] = timewait - g_current_time[id]
	g_remaining_time2[id] = timewait2 - g_current_time2[id]

	set_hudmessage(255, 255, 255, -1.0, 0.10, 0, 3.0, 3.0) // "[G] - %s (%i%%)^n[R] - %s (%i%%)"
	ShowSyncHudMsg(id, g_synchud1, "%L", LANG_PLAYER, "ZOMBIE_SKILL_DUO", zclass_desc, percent, zclass_desc2, percent2)
