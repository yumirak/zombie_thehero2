...
#define PLUGIN "[ZB3EX] ZClass: Regular"
#define VERSION "2.0"
#define AUTHOR "Dias"
...
new const Float:zclass_dmgmodifier = 1.0
...
{
	if( reset_time ) 
		g_current_time[id] = zb3_get_user_level(id) > 1 ? BERSERK_COOLDOWN_ORIGIN + BERSERK_TIME_ORIGIN : BERSERK_COOLDOWN_HOST + BERSERK_TIME_HOST
...
public client_PreThink(id)
{
	...
	if(!g_can_berserk[id] || g_berserking[id])
		return
	if(get_user_health(id) < HEALTH_DECREASE)
		return

	static Float:CurTime, Float:BotCurTime
	CurTime = get_gametime()
	if(CurTime - 0.5 > BotCurTime)
	{
		BotCurTime = CurTime
		static target, body;
